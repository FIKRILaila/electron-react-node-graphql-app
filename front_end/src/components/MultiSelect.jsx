...
export const MultiSelect = () => {
    ...
    useEffect(() => {
            if(data){
                setItems(data.Filiales.map(filiale => items.concat(filiale.name)))
            }
        },[data])
                                ...
                                <div className="flex flex-auto flex-wrap">
                                    {
                                        selectedItems.map((tag, index) => {
                                            return (
                                                <div key={index} className="flex justify-center items-center m-1 font-medium py-1 px-2  rounded-full text-sky-700 bg-sky-200 border border-sky-700 ">
